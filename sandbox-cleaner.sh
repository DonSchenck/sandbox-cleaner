...
oc delete $(oc get service.serving.knative.dev       -n "$project" -o name | grep "$app_prefix")
oc delete $(oc get route.serving.knative.dev         -n "$project" -o name | grep "$app_prefix")
oc delete $(oc get revision.serving.knative.dev      -n "$project" -o name | grep "$app_prefix")
